...
/*
 * Zen I/O Scheduler (blk-mq) - ANTI PANIC EDITION
 * FCFS, deadlines ensure fairness via safe Batching logic.
 */
...
#define ZEN_SYNC_BATCH 16 /* Batas maksimal Baca sebelum Tulis diselamatkan */
                                ...
                                struct list_head *list, bool at_head)
{
	...
	list_for_each_entry_safe(rq, next, list, queuelist) {
		...
		if (op_is_sync(rq->cmd_flags)) {
			if (at_head) list_add(&rq->queuelist, &zdata->sync_list);
			else list_add_tail(&rq->queuelist, &zdata->sync_list);
		} else {
			if (at_head) list_add(&rq->queuelist, &zdata->async_list);
			else list_add_tail(&rq->queuelist, &zdata->async_list);
		}
	}
...
static struct request *zen_dispatch_request(struct blk_mq_hw_ctx *hctx)
{
	...
	/* Logika Tenggat Waktu (Deadline) yang Aman:
	 * Jika sudah terlalu banyak BACA (Sync), beri jalan 1 untuk TULIS (Async) agar tidak hang.
	 */
	...
	/* Jika tidak, prioritaskan BACA (Sync) selamanya! */
...
static bool zen_has_work(struct blk_mq_hw_ctx *hctx)
{
	...
	spin_lock_irqsave(&zdata->lock, flags);
	has_work = !list_empty(&zdata->sync_list) || !list_empty(&zdata->async_list);
	spin_unlock_irqrestore(&zdata->lock, flags);
